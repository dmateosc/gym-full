...
// ─── Prompt builder ───────────────────────────────────────────────────────────
function buildPrompt(date, muscleGroups, goals, intensity, exercises) {
...
// ─── DB helpers ───────────────────────────────────────────────────────────────
async function getExercises(client) {
    const result = await client.query('SELECT id, name, category, muscle_groups FROM exercises ORDER BY name');
    return result.rows;
}
async function findRoutineByDate(client, date) {
    const result = await client.query('SELECT id FROM daily_routines WHERE routine_date = $1', [date]);
    return result.rows[0]?.id ?? null;
}
async function insertRoutine(client, data) {
    const result = await client.query(`INSERT INTO daily_routines
...
async function insertRoutineExercise(client, data) {
...
async function main() {
    ...
    try {
        ...
        const relevant = allExercises.filter((e) => e.muscle_groups?.some((mg) => schedule.muscleGroups.some((t) => mg.toLowerCase().includes(t.toLowerCase()) || t.toLowerCase().includes(mg.toLowerCase()))));
