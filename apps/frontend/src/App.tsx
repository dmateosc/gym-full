      ...
      >
        ...
        >
          <div className="container mx-auto px-3 sm:px-4">
            {/* Mobile: hamburguesa + nombre de sección */}
            <div className="sm:hidden flex items-center justify-between py-2">
              <button
                onClick={() => setDrawerOpen(true)}
                aria-label="Abrir menú"
                className="p-2 rounded-lg text-[#cbd5e1] hover:bg-white/5"
              >
              ...
              <span className="text-white text-sm font-semibold">
                {navigationTabs.find((t) => t.id === currentPage)?.label ?? ''}
              </span>
              <span className="w-9" aria-hidden />
            </div>

            {/* Desktop: tabs */}
            <nav className="hidden sm:flex gap-1 py-2 overflow-x-auto">
        ...
        {drawerOpen && (
          <div className="fixed inset-0 z-[60] sm:hidden">
            ...
            />
            <aside
              className="absolute left-0 top-0 bottom-0 w-72 max-w-[80vw] bg-[#0b1120] border-r border-white/10 shadow-2xl flex flex-col"
