...
// Maps muscle group names (Spanish) to SVG group IDs
const MUSCLE_TO_IDS: Record<string, string[]> = {
  'Pecho': ['chest'],
  'Bíceps': ['biceps'],
  'Bíceps Femoral': ['hamstrings'],
  'Tríceps': ['triceps'],
  'Hombros': ['front-shoulders', 'rear-shoulders'],
  'Deltoides': ['front-shoulders', 'rear-shoulders'],
  ...
  'Espalda': ['lats', 'lowerback', 'traps-middle'],
  'Dorsales': ['lats'],
  'Lumbar': ['lowerback'],
  ...
  'Pantorrillas': ['calves'],
  'Antebrazos': ['forearms'],
  'Cuerpo completo': ['chest', 'abdominals', 'quads', 'hamstrings', 'glutes', 'lats', 'front-shoulders', 'rear-shoulders'],
...
function buildSelectedSet(selectedMuscle?: string): Set<string> {
  if (!selectedMuscle) return new Set();
...
function getMuscleProps(
  ...
  const isSelected    = selected.has(svgId);
  const isInteractive = !!onMuscleClick;
...
const FrontBody: React.FC<{
  ...
  return (
    <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      <defs>
        <radialGradient id="jointradial-front" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
      ...
      {/* Front shoulders */}
      <g id="front-shoulders-front" {...mp('front-shoulders')}>
      ...
      <g id="body-outline-front" style={{ color: OUTLINE_COLOR }}>
        ...
        <path d="M282.82,83.56s.54-14.9,0-24.54c-.54-9.63,6.51-27.77,16.85-22.43,10.89,5.63,32.28,10.76,58.33,0,7.35-2.26,14.26,10.09,15.52,24.09,1.25,14,2.69,21.23,4.3,22.88" fill="none" stroke={OUTLINE_COLOR} strokeLinecap="round" strokeLinejoin="round" strokeWidth="3.46"/>
        <path d="M282.69,83.67c-5.36-3.86-3.61-20.07-6.05-37.39-2.56-18.19,16.27-32.16,24.89-34.59,5.86-1.65-18.28,1.29,6.39-6.76s33.37,2.13,37.66,3.41c3.74-.45-9.93-8.43,11.87-1.12,20.36,6.82,23.77,22.8,22.07,39.35-2.2,21.32,2.15,29.08-1.76,37.1" fill="none" stroke={OUTLINE_COLOR} strokeLinecap="round" strokeLinejoin="round" strokeWidth="3.46"/>
      </g>
    </svg>
  );
};
...
const BackBody: React.FC<{
  ...
  return (
    <svg viewBox="0 0 660.46 1206.46" fill="none" xmlns="http://www.w3.org/2000/svg" className="w-full h-full">
      <defs>
        <radialGradient id="jointradial-back" cx="50%" cy="50%" r="50%" fx="50%" fy="50%">
          <stop offset="0%" style={{ stopColor: 'rgb(254, 91, 127)', stopOpacity: 1 }} />
          <stop offset="100%" style={{ stopColor: 'rgb(231, 236, 239)', stopOpacity: 1 }} />
        </radialGradient>
      </defs>
...
const BodyMap: React.FC<BodyMapProps> = ({
  ...
  const highlighted = buildHighlightedSet(muscleGroups);
  const selected    = buildSelectedSet(selectedMuscle);
