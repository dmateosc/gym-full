...
const DetalleEjercicio = ({ ejercicio, onVolver }: DetalleEjercicioProps) => {
  const obtenerColorCategoria = (categoria: string) => {
    const colores = {
      fuerza: 'bg-gradient-to-r from-red-600 to-red-800 text-white border-red-500',
      cardio: 'bg-gradient-to-r from-orange-600 to-orange-800 text-white border-orange-500',
      flexibilidad: 'bg-gradient-to-r from-green-600 to-green-800 text-white border-green-500',
      resistencia: 'bg-gradient-to-r from-blue-600 to-blue-800 text-white border-blue-500',
      equilibrio: 'bg-gradient-to-r from-purple-600 to-purple-800 text-white border-purple-500',
      funcional: 'bg-gradient-to-r from-yellow-600 to-yellow-800 text-white border-yellow-500'
    };
    return colores[categoria as keyof typeof colores] || 'bg-gradient-to-r from-gray-600 to-gray-800 text-white border-gray-500';
  };

  const obtenerColorDificultad = (dificultad: string) => {
    const configs = {
      principiante: { bg: 'bg-gradient-to-r from-green-600 to-green-800', text: 'text-white', border: 'border-green-500' },
      intermedio: { bg: 'bg-gradient-to-r from-yellow-600 to-yellow-800', text: 'text-white', border: 'border-yellow-500' },
      avanzado: { bg: 'bg-gradient-to-r from-red-600 to-red-800', text: 'text-white', border: 'border-red-500' }
    };
        ...
        <div className="bg-gradient-to-r from-red-600 via-red-700 to-red-800 text-white p-8">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div className="mb-4 md:mb-0">
              ...
              <p className="text-red-100 text-lg">{ejercicio.descripcion}</p>
            </div>
            
            <div className="flex flex-col sm:flex-row gap-3">
              <span className={`px-4 py-2 rounded-full border shadow-lg ${obtenerColorCategoria(ejercicio.categoria)} font-medium`}>
                {ejercicio.categoria.charAt(0).toUpperCase() + ejercicio.categoria.slice(1)}
              </span>
              <span className={`px-4 py-2 rounded-full border shadow-lg ${dificultadConfig.bg} ${dificultadConfig.text} ${dificultadConfig.border} font-medium`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 p-8 bg-gradient-to-r from-gray-800 to-gray-900 border-b border-gray-700">
          {ejercicio.duracionEstimada && (
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br from-blue-600 to-blue-800 text-white rounded-full mb-2 shadow-lg">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12,2A10,10 0 0,0 2,12A10,10 0 0,0 12,22A10,10 0 0,0 22,12A10,10 0 0,0 12,2M12,4A8,8 0 0,1 20,12A8,8 0 0,1 12,20A8,8 0 0,1 4,12A8,8 0 0,1 12,4M12.5,7V12.25L17,14.92L16.25,16.15L11,13V7H12.5Z"/>
                </svg>
              </div>
              <p className="text-sm text-gray-400">Duración</p>
              <p className="text-lg font-semibold text-white">{ejercicio.duracionEstimada} min</p>
            </div>
          )}
          
          {ejercicio.calorias && (
            <div className="text-center">
              <div className="inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br from-orange-600 to-orange-800 text-white rounded-full mb-2 shadow-lg">
                <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
              ...
              <p className="text-lg font-semibold text-white">{ejercicio.calorias} cal</p>
            </div>
          )}
          
          <div className="text-center">
            <div className="inline-flex items-center justify-center w-12 h-12 bg-gradient-to-br from-green-600 to-green-800 text-white rounded-full mb-2 shadow-lg">
              <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            ...
            <p className="text-sm text-gray-400">Equipamiento</p>
            <p className="text-lg font-semibold text-white">{ejercicio.equipamiento[0]}</p>
          </div>
        </div>
          ...
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {ejercicio.instrucciones.map((instruccion, index) => (
                <div key={index} className="flex items-start space-x-4 p-4 bg-gradient-to-r from-gray-700 to-gray-800 rounded-lg border border-gray-600">
                  ...
                  <p className="text-gray-200 flex-1 pt-1">{instruccion}</p>
                </div>
              ))}
            </div>
          </div>
